// ERR-07: Bit Error Sequence
class can_bit_error_seq extends uvm_sequence#(wb_can_trans);
  `uvm_object_utils(can_bit_error_seq)

  function new(string name = "can_bit_error_seq");
    super.new(name);
  endfunction
  ...
  virtual task body();
    ...
    `uvm_info(get_name(), "=== ERR-07: BIT ERROR TEST START ===", UVM_LOW)
    ...
    wb_read(8'h0E, tec);
    `uvm_info(get_name(), $sformatf("Post-TX TEC = %0d", tec), UVM_LOW)
    
    wb_read(8'h0C, ecc_val);
    `uvm_info(get_name(), $sformatf("Post-TX ECC = 0x%0h", ecc_val), UVM_LOW)

    `uvm_info(get_name(), "=== ERR-07: BIT ERROR TEST COMPLETE ===", UVM_LOW)
  endtask
endclass

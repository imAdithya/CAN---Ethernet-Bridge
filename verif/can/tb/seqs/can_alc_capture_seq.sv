// PROT-07: Arbitration Lost Capture (ALC) Sequence
class can_alc_capture_seq extends uvm_sequence#(wb_can_trans);
  `uvm_object_utils(can_alc_capture_seq)

  function new(string name = "can_alc_capture_seq");
    super.new(name);
  endfunction
  ...
  virtual task body();
    logic [7:0] alc;
    `uvm_info(get_name(), "=== PROT-07: ALC CAPTURE TEST START ===", UVM_LOW)
    ...
    // Send an EFF frame (Frame Info = 0x82)
    wb_write(8'h10, 8'h82); 
    wb_write(8'h11, 8'h55); wb_write(8'h12, 8'h40); 
    wb_write(8'h13, 8'h11); wb_write(8'h14, 8'h22);
    wb_write(8'h01, 8'h10); // Self-TX
    
    // Wait for arbitration to be lost and transmission to stop
    #100000; 

    wb_read(8'h0B, alc);
    `uvm_info(get_name(), $sformatf("Post-TX ALC = 0x%0h", alc), UVM_LOW)
    
    // Abort any pending retries
    wb_write(8'h01, 8'h02); 
    #10000;

    `uvm_info(get_name(), "=== PROT-07: ALC CAPTURE TEST COMPLETE ===", UVM_LOW)
  endtask
endclass

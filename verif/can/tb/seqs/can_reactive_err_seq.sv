// ERR-10: Reactive Error Flag Sequence
class can_reactive_err_seq extends uvm_sequence#(wb_can_trans);
  `uvm_object_utils(can_reactive_err_seq)

  function new(string name = "can_reactive_err_seq");
    super.new(name);
  endfunction
  ...
  virtual task body();
    ...
    `uvm_info(get_name(), "=== ERR-10: REACTIVE ERROR TEST START ===", UVM_LOW)
    ...
    `uvm_info(get_name(), "=== ERR-10: REACTIVE ERROR TEST COMPLETE ===", UVM_LOW)
  endtask
endclass

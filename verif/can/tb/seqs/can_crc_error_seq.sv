// ERR-06: CRC Error Sequence
class can_crc_error_seq extends uvm_sequence#(wb_can_trans);
  ...
  virtual task body();
    ...
    `uvm_info(get_name(), "=== ERR-06: CRC ERROR TEST START ===", UVM_LOW)
    ...
    // To test CRC error, the UVM Agent physically driving the bus should corrupt the CRC field.
    // For now, we transmit a frame and verify REC/ECC behavior. If an external agent injects an error,
    // this sequence will capture it.
    ...
    `uvm_info(get_name(), "=== ERR-06: CRC ERROR TEST COMPLETE ===", UVM_LOW)
  endtask
endclass

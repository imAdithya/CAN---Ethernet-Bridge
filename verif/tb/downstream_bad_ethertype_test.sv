// DN-02: Bad EtherType Downstream Test
// Receives an Ethernet frame with wrong EtherType (not 0xCAFE/0xCABE).
// Verifies frame is dropped at VALIDATE, cnt_errors incremented, BD freed.
class downstream_bad_ethertype_test extends bridge_base_test;
  `uvm_component_utils(downstream_bad_ethertype_test)

  function new(string name = "downstream_bad_ethertype_test", uvm_component parent = null);
    super.new(name, parent);
  endfunction

  virtual task run_phase(uvm_phase phase);
    downstream_bad_ethertype_seq dn_seq;
    counter_verify_seq           cnt_seq;
    bit [31:0] rx_bd_addr, frame_ptr;

    phase.raise_objection(this);
    `uvm_info(get_type_name(), "Starting bad EtherType downstream test (DN-02)...", UVM_MEDIUM)
    #100ns;

    // Step 1: Configure bridge
    dn_seq = downstream_bad_ethertype_seq::type_id::create("dn_seq");
    dn_seq.start(m_env.m_host_agent.m_sequencer);

    // Step 2: Build frame with wrong EtherType (0x0800 = IPv4)
    rx_bd_addr = `DEFAULT_RX_BD_ADDR;
    frame_ptr  = rx_bd_addr + 32'h100;

    downstream_frame_builder::build_custom_ethertype_frame(
      m_mem_cfg, frame_ptr, 16'h0800
    );
    ...
    #500ns;
    `uvm_info(get_type_name(), "DN-02 finished.", UVM_MEDIUM)
